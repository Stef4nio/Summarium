package com.games440.summarium;

public class GameConfig {
    public static int CELLS_IN_VERTICAL = 9;
    public static int CELLS_IN_HORIZONTAL = 6;
    public static float TABLE_Y_OFFSET = 45;
    public static float TABLE_X_OFFSET = 40;
    public static float CELL_X_PADDING = 9.5f;
    public static float CELL_Y_PADDING = 9.5f;
    public static float SCREEN_WIDTH = 1080;
    public static float SCREEN_HEIGHT = 1920;
    public static float MIN_SCREEN_WIDTH = 1080;
    public static float MIN_SCREEN_HEIGHT = 1920;
    public static int MAX_AIM = 19;
    public static int MIN_AIM = 10;
    public static String isFirstLaunchEverKey = "isFirstLaunchEver";
}
